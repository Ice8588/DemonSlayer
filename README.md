...
這款遊戲的核心不是「揮得多快」，而是**看懂敵人身上浮現的方位提示，並用正確的刀鋒角度回應**：

- 🔴 **紅色提示** — 敵人接下來要攻擊的方向。在攻擊落下前把刀鋒轉到該角度就能**擋下**，超過時間則防禦失敗、承受傷害
- 🟡 **黃色提示** — 敵人沒有防備的**弱點**角度。把刀移到該處，再沿**對角線快速揮出**即可重創敵人

提示只存在數秒，於是每一場戰鬥都變成「辨識 → 抬臂到正確角度 → 擋下或揮出」的即時身體反應，而不是連打按鍵。
...
| **防禦** | 把合握的雙手移到紅色提示的角度並停住 | 刀鋒角度落入該方位區塊 |
| **攻擊** | 把雙手移到黃色提示的角度，再沿對角線快速揮出 | 起始與結束區塊相隔 3–5 格（約 135°–225°，接近對向） |
...
玩家走近敵人（預設 5 公尺內）時，敵人身上會浮現一圈**面朝玩家的環形介面**：中央是刀的當前指向，八個方位的提示分佈在圓環上；介面持續跟隨敵人，玩家離開範圍即自動收起。
...
- 生成的方位透過 [`QuadrantHintSpawner`](Assets/Scripts/Enemy/QuadrantHintSpawner.cs) 轉為圓環上的紅 / 黃提示

**提示的兩段生命週期**（[`QuadrantHint.cs`](Assets/Scripts/Enemy/QuadrantHint.cs)）

提示不是憑空出現又消失，而是用形變本身當作計時器：

1. **浮現期** — 以大面積、半透明的形態出現在圓環外，並逐漸向內收縮、轉為高亮。**此階段尚未生效**，玩家提早反應不算數
2. **生效期** — 完全收進圓環後才開放互動，接著繼續縮小、淡出。玩家若沒有在這段窗口內回應，提示就此消失 —— 紅色代表挨了一擊，黃色代表錯過一次破綻

於是「還能不能反應得及」這件事，玩家用眼睛看提示的大小就知道，不需要額外的進度條。

**玩家如何解題**（[`PlayerAttack.cs`](Assets/Player/PlayerAttack.cs)）

- **防禦**：把刀鋒轉到**紅色**方位並停住，即可擋下該次攻擊，敵人中斷動作並恢復移動
- **攻擊**：把刀移到**黃色**方位後，沿對角線快速揮出，命中弱點造成傷害（每次 20，敵人滿血 100）

攻擊之所以要求「對角線」，是因為揮砍不是單點觸發，而是**一段跨區塊的軌跡**：系統取刀鋒的起始與結束區塊，只有兩者相隔 3–5 格（接近對向）時才算一次有效揮擊，避免手臂的細微晃動被誤判成攻擊。
